...
SELECT id, created_at, updated_at, body, user_id FROM posts ORDER BY created_at ASC
`
...
const getPostFromUserId = `-- name: GetPostFromUserId :many
SELECT id, created_at, updated_at, body, user_id FROM posts WHERE user_id = $1 ORDER BY created_at ASC
`

func (q *Queries) GetPostFromUserId(ctx context.Context, userID uuid.UUID) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getPostFromUserId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
